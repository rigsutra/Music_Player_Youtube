...
export default function useUploadProgress(uploadId, onComplete) {
  ...
  useEffect(() => {
    ...
    const sseUrl = `${API_BASE}/api/upload/progress/${uploadId}/stream?token=${encodeURIComponent(
      token
    )}`;
    ...
    // Track if we've already dispatched the completion event
    ...
    evtSource.onmessage = (e) => {
      try {
        ...
        if (data.stage === "done" && !hasDispatched) {
          hasDispatched = true;
          ...
          (data.stage === "error" || data.stage === "canceled") &&
          !hasDispatched
        ) {
          hasDispatched = true;
