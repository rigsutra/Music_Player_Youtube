...
// /utils/youtube.js - Production version for Render deployment
const ytdl = require('@distube/ytdl-core');
const youtubedl = require('youtube-dl-exec');
const { spawn } = require('child_process');
const { PassThrough } = require('stream');
...
// Method 1: Using youtube-dl-exec with proper handling for both local and server
async function streamWithYoutubeDlExec(url, onProgress) {
  try {
    console.log('🎵 Attempting download with youtube-dl-exec...');
    ...
    // Check for cookies - Render uses /etc/secrets/cookies.txt
    const cookiePaths = [
      '/etc/secrets/cookies.txt',  // Render secret files location
      './cookies.txt',              // Local development
      process.env.YOUTUBE_COOKIES_FILE  // Custom path from env
    ].filter(Boolean);

    let cookieFileFound = false;
    ...
    if (!cookieFileFound) {
      console.log('🍪 No cookies file found, proceeding without cookies');
      // Don't use browser cookies on server - it causes issues
      if (process.env.NODE_ENV !== 'production') {
        // Only try browser cookies in local development
        options.cookiesFromBrowser = 'chrome';
      }
    }

    // Use exec to get the subprocess
    const subprocess = youtubedl.exec(url, options);
    ...
    subprocess.stderr.on('data', (data) => {
      ...
      if (text.includes('ERROR') && !text.includes('Failed to decrypt with DPAPI')) {
    ...
    subprocess.on('exit', (code) => {
      if (code === 0) {
        ...
        console.warn(`youtube-dl-exec exited with code ${code}`);
      }
    });
...
// Method 2: Using system yt-dlp (fallback)
function streamWithSystemYtDlp(url, onProgress) {
  return new Promise((resolve, reject) => {
    console.log('🎵 Attempting download with system yt-dlp...');
    ...
    const cookiePaths = ['/etc/secrets/cookies.txt', './cookies.txt', process.env.YOUTUBE_COOKIES_FILE].filter(Boolean);
    for (const cookiePath of cookiePaths) {
      if (fs.existsSync(cookiePath)) {
        args.push('--cookies', cookiePath);
        break;
    ...
    if (!found) {
      reject(new Error('No youtube downloader found in system'));
    }
  });
}
...
async function getVideoInfo(url) {
  ...
  try {
    ...
    console.warn('⚠️ Video info failed, using defaults:', error.message);
    return {
      title: 'Unknown Title',
      duration: null,
      thumbnail: null
